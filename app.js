...
const SUPABASE_URL = 'https://db2.74club.ru';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJyb2xlIjoiYW5vbiIsImlhdCI6MTc4NjY3ODU5NSwiZXhwIjoyMTAyMDM4NTk1fQ.fjLLCoGKvvGu6Hb_e_beiWh5-S2MUQP8wOvnBg7fUGE';
...
/************************************************************
 * v3 AUTH (свой логин через RPC login()/change_password(),
 * без GoTrue — облегчённый self-hosted стек)
 ************************************************************/

const ADMIN_TOKEN_KEY = 'pokerAdminToken';
...
function rebuildSupabaseClient(token) {
    ...
    supabaseClient = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY, options);
}

async function adminLogin(password) {
    if (!supabaseClient) return { error: 'База не готова' };
...
async function initSupabase() {
    ...
    // Восстанавливаем сессию админа по сохранённому токену, если он
    // ещё не истёк (работает в любом браузере после однократного входа).
...
const SEAT_PRESETS = {
    ...
    7: [
        { x: 22, y: 14 }, { x: 50, y: 14 }, { x: 78, y: 14 },
        { x: 95, y: 50 },
        { x: 65, y: 86 }, { x: 35, y: 86 },
        { x: 5, y: 50 }
    ],
    8: [
        { x: 18, y: 14 }, { x: 50, y: 14 }, { x: 82, y: 14 },
        { x: 95, y: 50 },
        { x: 82, y: 86 }, { x: 50, y: 86 }, { x: 18, y: 86 },
        { x: 5, y: 50 }
    ],
    9: [
        { x: 15, y: 14 }, { x: 38, y: 14 }, { x: 62, y: 14 }, { x: 85, y: 14 },
        { x: 95, y: 50 },
        { x: 70, y: 86 }, { x: 30, y: 86 },
        { x: 5, y: 50 }, { x: 5, y: 15 }
    ],
    10: [
        { x: 15, y: 14 }, { x: 38, y: 14 }, { x: 62, y: 14 }, { x: 85, y: 14 },
        { x: 95, y: 50 },
        { x: 85, y: 86 }, { x: 62, y: 86 }, { x: 38, y: 86 }, { x: 15, y: 86 },
        { x: 5, y: 50 }
    ]
};
 ...
 ************************************************************/
   ...
   function bindEvents() {
    ...
    $('changePasswordBtn').onclick = async () => {
        ...
        const { error } = await supabaseClient.rpc('change_password', { new_password: p1 });
